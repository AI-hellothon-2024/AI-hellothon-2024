...
async def llm_scenario_create(job, situation, gender, before_scenario_content, scenario_step, user_id, before_settings):
    ...
    if scenario_step == "1":
        # 첫 번째 대화 프롬프트
        prompt = (
            f"나는 다음의 조건에 따라서 롤플레잉 대화를 진행하고 싶다.\n"
            f"- 내 직업은 '{job}'이고, 내 성별은 '{gender}'이다.\n"
            f"- 상황은 '{situation_description}'이며, 대화는 이 상황을 기반으로 전개된다.\n"
            f"- 상대는 내 성별의 반대 성별이고, 성격은 무작위로 설정된다. (예: 친절하거나, 도발적이거나, 유머러스하거나, 성격이 더럽거나, 무모하거나 등)\n"
            f"- 대화는 자연스럽게 발전하며 최종 상황에 도달하기 전에 대화를 천천히 빌드업한다.\n"
            f"- 상대방은 매번 한 마디씩 대화를 진행한 뒤 내 대답을 기다려야 한다.\n\n"
            f"**응답 형식:** 아래 형식에 따라 작성하라.\n"
            f"- (임의로 정해진 상대의 설정값) setting::: 이 부분에 상대의 설정값을 넣어줘\n"
            f"- (대화 시작) start::: 이 부분에 상대방의 첫 대사를 넣어줘\n\n"
            f"**금지 사항:**\n"
            f"- ‘(’, ‘)’와 같은 문자는 사용하지 마라.\n"
            f"- 대답이 대화 흐름에 맞지 않으면 첫 줄에 'end'라고 적어라."
        )
    else:
        # 후속 대화 프롬프트
        prompt = (
            f"나는 다음의 조건에 따라서 롤플레잉 대화를 진행하고 싶다.\n"
            f"- 내 직업은 '{job}'이고, 내 성별은 '{gender}'이다.\n"
            f"- 상대방의 설정값: {before_settings}\n"
            f"- 상황은 '{situation_description}'이다.\n"
            f"- 지금까지의 대화 내용은 다음과 같다:\n"
            f"  {before_scenario_content}\n\n"
            f"위 내용을 참고하여, 다음 조건에 따라 대화를 진행해라:\n"
            f"1. 현재 대화 흐름을 기반으로 새로운 대화를 생성하라.\n"
            f"2. 대화는 상대방의 설정값과 현재 상황에 맞게 작성되어야 한다.\n"
            f"3. 대화는 자연스럽게 전개되며, 흥미로운 전환점을 만들어야 한다.\n"
            f"4. 응답 형식에 반드시 따르며 금지된 표현은 사용하지 않는다.\n\n"
            f"**응답 형식:** 아래 형식에 따르도록 작성하라.\n"
            f"- (임의로 정해진 상대의 설정값) setting::: 이 부분에 상대의 설정값을 넣어줘 (이전 설정값과 동일해야 함)\n"
            f"- (대화 시작) start::: 이 부분에 상대방의 다음 대사를 넣어줘\n\n"
            f"**금지 사항:**\n"
            f"- ‘(’, ‘)’와 같은 문자는 사용하지 마라.\n"
            f"- 대답이 대화 흐름에 맞지 않거나 대화 종료를 유도해야 한다고 판단되면 응답의 첫 줄에 'end'라는 단어를 추가하라."
        )
